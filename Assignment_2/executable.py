...
def main():
    ...
    cap = cv.VideoCapture('foreground1.avi')
    ...
    for n_frame in range(0,num_frames):
        nframe=n_frame
...
def key_callback(window, key, scancode, action, mods):
    ...
    # When the first frame goes no need to press G again
    if pressed_yes>2:
        key = glfw.KEY_G
        action = glfw.PRESS

    if key == glfw.KEY_G and action == glfw.PRESS:
        pressed_yes=pressed_yes+1
        global cube
        nframe = nframe + 1
        positions, colors = set_voxel_positions(config['world_width'], config['world_height'], config['world_width'], nframe)
